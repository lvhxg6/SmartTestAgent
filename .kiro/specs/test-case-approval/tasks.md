...
- [ ] 4. 实现重新生成功能
  - [ ] 4.1 添加 regenerateTestCases API
...
- [ ] 5. 实现 Playwright 脚本生成器
  - [ ] 5.1 创建 ScriptGenerator 组件
    - 创建 `packages/core/src/script-generator/index.ts`
    - 实现 `generateForTestCase` 方法
    - 实现 `generateForTestCases` 方法
    - 实现 `generateExecutableScript` 方法
    - _Requirements: 6.1, 6.5_
  
  - [ ] 5.2 添加 generateScript API
    - 在 `apps/server/src/trpc/routers/testRun.ts` 中添加路由
    - 调用 ScriptGenerator 生成脚本
    - 返回脚本内容
    - _Requirements: 6.1, 6.3, 8.3_
  
  - [ ] 5.3 添加 downloadScript API
    - 支持单文件下载（.js）
    - 支持多文件打包下载（.zip）
    - 添加注释说明
...
- [x] 7. 实现前端审批页面
  ...
  - [x] 7.2 创建 TestCaseList 组件
    - 创建 `apps/web/src/components/TestCaseList.tsx`
    - 使用 Collapse 组件展示测试用例
    - 支持按需求筛选
    - _Requirements: 2.1, 2.3, 2.4_
  
  - [x] 7.3 创建 TestCaseDetail 组件（内嵌在 TestCaseList 中）
    - 显示测试用例完整信息（步骤、断言等）
    - _Requirements: 2.2_
  
  - [x] 7.4 创建 ApprovalActions 组件
    - 创建 `apps/web/src/components/ApprovalActions.tsx`
    - 添加批准、拒绝、预览脚本、下载脚本按钮
    - _Requirements: 4.1, 4.2, 4.3_
  
  - [x] 7.5 创建 RejectFeedbackModal（内嵌在 ApprovalActions 中）
    - 支持选择反馈类型
    - 支持填写详细反馈
    - _Requirements: 4.3, 4.4_
  
  - [x] 7.6 创建 ScriptPreviewModal 组件
